...
func TestBusca(t *testing.T) {
	...
	t.Run("palavra conhecida", func(t *testing.T) {
		...
		compraraStrings(t, resultado, esperado)
	})
...
func compraraStrings(t *testing.T, resultado, esperado string) {
	t.Helper()
...
// Declaração do map começa com a palavra-chave map e requer dois tipos. O primeiro é o tipo da chave, que é escrito dentro de [].
// O segundo é o tipo do valor, que vai logo após o [].
// O tipo da chave é especial. Só pode ser um tipo comparável, porque sem a habilidade de dizer se duas chaves são iguais, não temos
// como ter certeza de que estamos obtendo o valor correto.
